...
Before spawning investigators, list the MCP servers this session has. Their tools appear as `mcp__<server>__<tool>`, and a deferred server is listed by name for `ToolSearch` to load.
...
On this machine that usually resolves to two live categories: source control through git and `gh`, and long-form documents through Google Drive. Name the other five as gaps in the coverage map rather than spawning an investigator with no tool to query.
...
Not the `Explore` agent: an investigator needs the MCP tools, and reading only is an instruction here rather than a sandbox.
...
- `subagent_type`: `general-purpose`, told to read only and never edit or write files
- `model`: `fable`. Its quality check spot-verifies citations, so it needs the same MCP access the investigators had.
