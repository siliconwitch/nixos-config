...
- **Doctor:** one read-only check that answers "is this instance worth driving?" It confirms process up, right version/build, port owned by us, auth valid. An agent runs this first whenever anything looks off.
...
When the surface is hardware, the generated skill names every physical action it takes (flashing, power sequencing, actuator movement) and gates anything a reflash cannot undo behind a human check.
