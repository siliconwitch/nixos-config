...
- `subagent_type`: `Explore` (read-only, it has no Edit or Write)
- `model`: `haiku` for a narrow slice, `sonnet` for a broad one
...
After the explanation is complete, spawn one architectural critic per model in the critic list (`fable`, `opus`, `sonnet`), all in a single message. For a critic outside the Claude family, hand one to Codex through the **delegate** skill.

For each critic:
- `subagent_type`: `general-purpose`, told to read only and never edit or write files
- `model`: one model from the critic list. Escalate to `opus` when the architecture warrants deeper analysis.
