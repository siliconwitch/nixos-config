...
description: "Periodic pass that keeps a project's verification skill and feature map honest: parallel source readers per feature, one live session driving every feature, at most one branch of proven corrections. Use for /maintain-verification-skill or \"audit the verify skill\"."
...
- **changed**: proven doc, harness, or map corrections land as a commit on a branch.
...
6. **Ship or stop.** For changed: commit the proven corrections on a branch, re-read every changed file first, and report the branch. Pushing or opening a PR waits for the user to ask. For clean or blocked: no commit, report the outcome and the coverage honestly.
